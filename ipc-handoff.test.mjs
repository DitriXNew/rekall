...
function unixSocketFixture({ platform, env = {}, directory = {}, socket = {}, getuid = () => 501 } = {}) {
  const seen = [];
  const options = { platform, env, homedir: () => platform === 'linux' ? '/home/synthetic' : '/Users/synthetic', getuid,
    lstatSync: file => {
      seen.push(file);
...
for (const platform of ['darwin', 'linux']) {
  test(`${platform} IPC resolves the private default or explicit Codex home`, () => {
    const home = platform === 'linux' ? '/home/synthetic' : '/Users/synthetic';
    for (const [env, expected] of [[{}, `${home}/.codex/ipc/ipc.sock`],
      [{ CODEX_HOME: '/private/custom codex' }, '/private/custom codex/ipc/ipc.sock']]) {
      const { options, seen } = unixSocketFixture({ platform, env });
      assert.equal(resolveIpcEndpoint(options), expected);
      assert.deepEqual(seen, [expected.slice(0, -9), expected]);
    }
    assert.throws(() => resolveIpcEndpoint(unixSocketFixture({ platform, env: { CODEX_HOME: 'relative' } }).options), /absolute/);
  });

  test(`${platform} IPC rejects foreign, exposed, or symlinked directories and sockets`, () => {
    for (const directory of [{ uid: 502 }, { mode: 0o755 }, { mode: 0o770 }, { isDirectory: () => false }]) {
      const { options, seen } = unixSocketFixture({ platform, directory });
    ...
    for (const socket of [{ uid: 502 }, { mode: 0o666 }, { mode: 0o640 }, { isSocket: () => false }]) {
      assert.throws(() => resolveIpcEndpoint(unixSocketFixture({ platform, socket }).options), /socket must be private/);
    }
    assert.throws(() => resolveIpcEndpoint(unixSocketFixture({ platform, getuid: () => undefined }).options), /verify.*user/);
  });

  test(`missing ${platform} socket fails without falling back to another endpoint`, () => {
    const { options } = unixSocketFixture({ platform });
    options.lstatSync = () => { throw Object.assign(new Error('missing'), { code: 'ENOENT' }); };
    assert.throws(() => resolveIpcEndpoint(options), { code: 'ENOENT' });
  });

  test(`${platform} endpoint checks work with real isolated Unix sockets and symlinks`,
    { skip: process.platform === 'win32' }, async t => {
      ...
      const options = { platform, env: { CODEX_HOME: root } };
...
test('unsupported platforms fail before inspecting an endpoint', () => {
  assert.throws(() => resolveIpcEndpoint({ platform: 'freebsd',
    lstatSync: () => { throw new Error('Unexpected filesystem lookup'); } }), /unsupported on freebsd/);
});
